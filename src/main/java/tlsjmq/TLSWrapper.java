 ...
 * <p/>
 * {@link TLSWrapper} implements the handshake protocol, required to establish a connection between two peers,
 * which is common for both client and server and provides the abstract {@link TLSWrapper#read(JMQChannel, SSLEngine)} and
 * {@link TLSWrapper#write(JMQChannel, SSLEngine, String)} methods, that need to be implemented by the specific SSL/TLS peer
 * that is going to extend this class.
 *
 * @author <a href="mailto:alex.a.karnezis@gmail.com">Alex Karnezis</a>
 */
public abstract class TLSWrapper {
    ...
    protected abstract void read(JMQChannel jmqChannel, SSLEngine engine) throws Exception;

    protected abstract void write(JMQChannel jmqChannel, SSLEngine engine, String message) throws Exception;
     ...
     *
     * @param JMQChannel - the socket channel that connects the two peers.
     ...
     */
    protected boolean doHandshake(JMQChannel jmqChannel, SSLEngine engine) throws IOException {
        ...
        while (handshakeStatus != SSLEngineResult.HandshakeStatus.FINISHED && handshakeStatus != SSLEngineResult.HandshakeStatus.NOT_HANDSHAKING) {
            ...
            case NEED_UNWRAP:
                log.debug(">>>>unwrap");
                int ret = jmqChannel.read(peerNetData);
                log.debug("read:" + peerNetData.position());
                if (ret < 0) {
                ...
                log.debug("unwrap status:"+result.getStatus());
            ...
            case NEED_WRAP:
                ...
                log.debug("wrap status:"+result.getStatus());
                switch (result.getStatus()) {
                    case OK :
                        myNetData.flip();
                        while (myNetData.hasRemaining()) {
                            log.debug("write1:" + myNetData.remaining());
                    ...
                    case CLOSED:
                        try {
                            myNetData.flip();
                            while (myNetData.hasRemaining()) {
                                log.debug("write2:" + myNetData.remaining());
                                jmqChannel.write(myNetData);
                            }
     ...
     */
    protected void closeConnection(JMQChannel jmqChannel, SSLEngine engine) throws IOException  {
        engine.closeOutbound();
        doHandshake(jmqChannel, engine);
        jmqChannel.close();
    }
     ...
     * @throws IOException if an I/O error occurs to the socket channel.
     */
    protected void handleEndOfStream(JMQChannel jmqChannel, SSLEngine engine) throws IOException  {
        try {
            engine.closeInbound();
        } catch (Exception e) {
            log.error("This engine was forced to close inbound, without having received the proper SSL/TLS close notification message from the peer, due to end of stream.");
