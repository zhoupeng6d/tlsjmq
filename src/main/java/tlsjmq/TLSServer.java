 ...
 * </p>
 * TLSServer makes use of Java NIO, and specifically listens to new connection requests with a {@link ServerSocketChannel}, which will
 * create new {@link SocketChannel}s and a {@link Selector} which serves all the connections in one thread.
 *
 * @author <a href="mailto:alex.a.karnezis@gmail.com">Alex Karnezis</a>
 */
public class TLSServer extends TLSWrapper {
     ...
     * @throws Exception
     */
    public TLSServer(String protocol, String addr) throws Exception {
        ...
        */
        ...
        ZMQ.Socket socket = jmqContext.createSocket(SocketType.REP);
        socket.bind(addr);
     ...
     * @throws Exception
     */
    private void accept(JMQChannel jmqChannel, SSLEngine engine) throws Exception {

    	log.debug("New connection request!");

        if (!doHandshake(jmqChannel, engine)) {
            jmqChannel.close();
     ...
     */
    @Override
    protected void read(JMQChannel jmqChannel, SSLEngine engine) throws IOException {
        ...
        if (bytesRead > 0) {
            ...
            write(jmqChannel, engine, "Hello! I am your server!");
